...
def _sum_neighbors(
    compact_lookup: dict[str, tuple[float, int]], centers: List[str], k: int
):
    """
...
def aggregate_clusters(df_hex: pd.DataFrame, k: int) -> pd.DataFrame:
    ...
    base["aantal_huizen"] = (
        pd.to_numeric(base["aantal_huizen"], errors="coerce").fillna(0).astype("int32")
    )

    lookup = {
        str(row.h3_index): (
            float(row.gemiddeld_jaarverbruik_mWh),
            int(row.aantal_huizen),
        )
    ...
    sums = pd.DataFrame(
        records, columns=["h3_index", "cluster_MWh", "cluster_buildings"]
    )
...
def shortlist_centers(
    df_hex: pd.DataFrame, threshold_kwh_m2: float, top_frac: float = 0.6
) -> pd.DataFrame:
    """
    Houd alleen hexen:
    - Boven drempel (kWh/m²)
    ...
    s["kWh_per_m2"] = (
        pd.to_numeric(s["kWh_per_m2"], errors="coerce").fillna(0).astype("float32")
    )
    s["gemiddeld_jaarverbruik_mWh"] = (
        pd.to_numeric(s["gemiddeld_jaarverbruik_mWh"], errors="coerce")
        .fillna(0)
        .astype("float32")
    )
...
def compute_clusters_cached(
    cache_key: str, df_hex_small: pd.DataFrame, k: int, ttl: int = 1800
) -> pd.DataFrame:
    """
...
def select_sites_from_clusters(
    ...
    out["cluster_MWh"] = (
        pd.to_numeric(out["cluster_MWh"], errors="coerce").fillna(0).astype("int32")
    )
    out["cluster_buildings"] = (
        pd.to_numeric(out["cluster_buildings"], errors="coerce")
        .fillna(0)
        .astype("int32")
    )
    ...
    out["connected_buildings"] = (
        out["cluster_buildings"].clip(upper=cap_bld_i).astype("int32")
    )
    ...
    for r in cand.itertuples():
        ...
        too_close = any(
            (h3.grid_distance(r.h3_index, b) is not None)
            and (h3.grid_distance(r.h3_index, b) <= int(min_sep_cells))
            for b in blocked
        )
        if too_close:
            continue
