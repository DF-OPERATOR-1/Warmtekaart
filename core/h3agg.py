...
def ensure_parent_series_for(
    df_with_res13: pd.DataFrame, res: int, cache: dict
) -> pd.Series:
    """
...
def build_res13_agg(df_points_res13: pd.DataFrame) -> pd.DataFrame:
    ...
    kwh_sum = (
        pd.to_numeric(tmp["kWh_per_m2"], errors="coerce").fillna(0).astype("float32")
    )
...
@st.cache_data(show_spinner=False, max_entries=2, ttl=240)
def rollup_to_resolution(
    res13_agg: pd.DataFrame, target_res: int, _cache_key: int = 0
) -> pd.DataFrame:
    """
    ...
    if target_res == BASE_H3_RES:
        ...
        parents = res13_agg[H3_RES13_COL].map(
            lambda h: h3.cell_to_parent(h, target_res)
        )
